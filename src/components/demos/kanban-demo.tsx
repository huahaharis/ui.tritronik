...
const initialData: Column[] = [
  ...
  },
  {
    id: "in-progress",
    title: "In Progress",
    tasks: [{ id: "t3", title: "Implement KanbanBoard", tag: "Dev" }],
  },
  {
    id: "done",
    title: "Done",
    tasks: [{ id: "t4", title: "Create Plan", tag: "Planning" }],
  },
];
...
export function KanbanDemo() {
  const {
    ...
    handleDragEnd,
    sensors,
  } = useKanban<Task, Column>(initialData, (newColumns) => {
    console.log("State changed! Syncing with backend...", newColumns);
    // await api.updateBoard(newColumns);
  });
        ...
        {columns.map((col) => (
          <KanbanColumn key={col.id} id={col.id} className="w-[300px]">
            ...
            <KanbanList
              ...
              className="flex-col gap-2 min-h-[100px]"
            >
              {col.tasks.map((task) => (
                <KanbanCard
                  key={task.id}
                  id={task.id}
                  className="p-3 bg-card border rounded-md shadow-sm hover:shadow-md transition-all"
                >
        ...
        >
          {col.tasks.map((task) => (
            <KanbanCard
              key={task.id}
              id={task.id}
              className="group flex items-center justify-between p-3 bg-card border rounded-md shadow-sm hover:border-primary/50 transition-colors"
            >
        ...
        >
          {col.tasks.map((task) => (
            <KanbanCard
              key={task.id}
              id={task.id}
              className="aspect-square flex flex-col items-center justify-center p-4 bg-muted/30 border-2 border-dashed rounded-xl hover:border-primary/50 hover:bg-muted/50 transition-all cursor-grab group"
            >
